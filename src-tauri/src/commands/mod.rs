...
/// Frontend → backend diagnostic ping so the unified log can show whether the overlay
/// webview is still alive and receiving show/hide events (e.g. after a paste).
#[tauri::command]
#[specta::specta]
pub fn overlay_ack(label: String) {
    log::info!("[overlay-fe] {}", label);
}
