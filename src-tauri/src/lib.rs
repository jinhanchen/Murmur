...
pub fn run(cli_args: CliArgs) {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            // The main window creates the WebView2 environment that the overlay SHARES
            // (same data_directory), and browser args are fixed by whichever window
            // creates it first. Disable native occlusion detection here so the transparent
            // always-on-top capsule keeps painting after a paste changes foreground —
            // otherwise WebView2 marks the overlay occluded and SUSPENDS rendering, so the
            // capsule never reappears. (The env var WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS
            // is ignored because wry always sets AdditionalBrowserArguments explicitly.)
            // The first three features mirror wry's defaults so nothing else regresses.
            #[cfg(target_os = "windows")]
            {
