...
impl TranscriptionManager {
    ...
    pub fn transcribe(&self, audio: Vec<f32>) -> Result<String> {
        ...
        // Guard against Whisper hallucinating text from silence/near-silence. The audio
        // is already VAD-filtered (16kHz mono), but stray noise frames can slip through
        // when nothing was actually said. If what's left is too short or too quiet to be
        // real speech, return empty so nothing downstream gets pasted.
        const MIN_SPEECH_SAMPLES: usize = 16_000 / 4; // 0.25s @ 16kHz
        const MIN_SPEECH_RMS: f32 = 0.005;
        let rms = (audio.iter().map(|s| s * s).sum::<f32>() / audio.len() as f32).sqrt();
        if audio.len() < MIN_SPEECH_SAMPLES || rms < MIN_SPEECH_RMS {
            debug!(
                "Audio below speech threshold (~{:.2}s, rms={:.4}); skipping to avoid hallucination",
                audio.len() as f32 / 16_000.0,
                rms
            );
            self.maybe_unload_immediately("no speech");
            return Ok(String::new());
        }
