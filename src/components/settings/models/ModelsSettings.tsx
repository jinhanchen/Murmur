    ...
    <div className="max-w-3xl w-full mx-auto space-y-4">
      ...
      {filteredModels.length > 0 ? (
        <div className="space-y-6">
          ...
          {/* Available Models Section — collapsed by default to keep the page short */}
          {availableModels.length > 0 && (
            <div className="space-y-3">
              <button
                type="button"
                onClick={() => setAvailableExpanded((v) => !v)}
                className="flex items-center gap-1.5 text-sm font-medium text-text/60 hover:text-text transition-colors cursor-pointer"
              >
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${
                    availableExpanded ? "" : "-rotate-90"
                  }`}
                />
                <span>{t("settings.models.availableModels")}</span>
                <span className="text-text/40">({availableModels.length})</span>
              </button>
              {availableExpanded &&
